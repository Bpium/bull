...
*/
  ...
  moveToFinished(
    ...
    return job.queue.client.moveToFinished(args).then(result => {
      if (result < 0) {
        const error = scripts.finishedErrors(result, job.id, 'finished', 'active');

        // console.error(`moveToFinished error: code=${result}, jobId=${job.id}, target=${target}`, {
        //   jobState: job,
        //   args: args.slice(0, 5)
        // });
  ...
  moveToDelayedArgs(queue, jobId, timestamp, ignoreLock) {
    ...
    const keys = _.map(
      ['active', 'delayed', jobId, 'stalled'],
      name => queue.toKey(name)
    );
    
    // Добавляем KEYS[5] - rate limiter key
    keys.push(queue.keys.limiter || '');
  
    const args = [
      JSON.stringify(timestamp),  // ARGV[1]
      jobId,                       // ARGV[2]
      ignoreLock ? '0' : queue.token  // ARGV[3]
    ];
  
    // Добавляем параметры rate limiter
    if (queue.limiter) {
      args.push(
        queue.limiter.groupKey ? 'true' : '',  // ARGV[4] - rate limit group key
        queue.limiter.mode || 'time'           // ARGV[5] - rate limit mode
      );
    }
      ...
      switch (result) {
        ...
        case -3:
          throw new Error(
            'Job ' +
              jobId +
              ' is not in the active state when trying to move to delayed'
          );
      }
    });
  },
 ...
 moveUnlockedJobsToWait(queue) {
  ...
  retryJobArgs(job, ignoreLock) {
    ...
    // Добавляем KEYS[8] - rate limiter key
    keys.push(queue.keys.limiter || '');
    ...
    const args = [
      pushCmd,                              // ARGV[1]
      jobId,                                // ARGV[2]
      ignoreLock ? '0' : job.queue.token    // ARGV[3]
    ];
