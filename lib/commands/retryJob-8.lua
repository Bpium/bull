--[[
  Retries a failed job by moving it back to the wait queue.
    Input:
      ...
      KEYS[6] stalled key
      KEYS[7] 'priority'
      KEYS[8] rate limiter key

      ARGV[1]  pushCmd
      ARGV[2]  jobId
      ARGV[3]  token
      ARGV[4]  rate limit group key (optional)
      ARGV[5]  rate limit mode (optional)
...
if rcall("EXISTS", KEYS[3]) == 1 then
    ...
    -- Декремент rate limiter после успешного удаления из active
    decrementRateLimiter(KEYS[8], ARGV[2], ARGV[4], ARGV[5])
